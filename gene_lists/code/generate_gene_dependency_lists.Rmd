...
vals <- 'negative' # 'both' (for all values, positive and negative) or 'negative' (for negative values only)
output_dir <- 'results/'
...
selected_tissues <- c('all', 'bladder', 'breast', 'cervix', 'ovary', 'liver', 'colo', 'esophag', 'head|neck', 'kidney', 'liver', 'lung', 'prosta', 'rect', 'stomach', 'thyroid')
```
...
cell_lines[inds_of_tissue_cell_lines] <- paste(cell_lines[inds_of_tissue_cell_lines], cell_line_data[inds_of_tissue_cell_lines, 'Tissue'], sep='_')
...
scores_filename <- paste(depMap_files_dir, file_prefix, '.', file_extension, sep='')
