...
bc_files <- paste(bc_dir, bc_names, '_bc_values.csv', sep='')
...
plot_df <- function(df, plot_title='', colors=c('black')){
  ...
  ggtitle(plot_title) +
  xlab(toTitleCase(gsub('_', ' ', x_feature))) +
  ylab(toTitleCase(gsub('_', ' ', y_feature))) +
...
get_plot_title <- function(first_feature_name, second_feature_name, first_names_list, second_names_list, i, j){
...
# first_file_list <- expression_files; first_names_list <- expression_names; first_feature_name <- 'DE LogFC'
# first_gene_col <- expression_gene_col; first_feature_col <- expression_feature_col

first_file_list <- bc_files; first_names_list <- bc_names; first_feature_name <- 'Betweennes Centrality'
first_gene_col <- bc_gene_col; first_feature_col <- bc_feature_col

second_file_list <- lit_gene_files; second_names_list <-lit_gene_names; second_feature_name <- 'Citation Count'
second_gene_col <- lit_gene_col; second_feature_col <- lit_feature_col
...
for(i in 1:length(first_file_list)) {
  ...
  for(j in 1:length(second_file_list)){
    ...
    plot_title <- get_plot_title(first_feature_name, second_feature_name, first_names_list, second_names_list, i, j)
    print(plot_title)
    ...
    ps[[plot_index]] <- plot_df(common_df, plot_title=plot_title, colors=colors)
    plot_index <- plot_index+1
  }
}

ps_grid <- ggarrange(plotlist=ps, nrow=sqrt(length(ps)), ncol=ceiling(sqrt(length(ps))))
